...
import { getFirebaseAdminCredentials, getFirebaseProjectId, usingEmulators } from './env';
...
function ensureInitialized() {
  ...
  if (usingEmulators()) {
    ...
    initializeApp({ projectId: getFirebaseProjectId() });
    return;
  }

  initializeApp({ credential: cert(getFirebaseAdminCredentials()) });
}
